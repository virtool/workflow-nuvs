...
async def eliminate_otus(
    ...
    paths = [str(sample.read_paths[0])]

    if sample.paired:
        paths.append(str(sample.read_paths[1]))
...
@step
async def reunite_pairs(sample: Sample, work_path: Path, logger):
    if sample.paired:
        headers = await read_fastq_headers(work_path / "unmapped_hosts.fq")

        unmapped_roots = {h.split(" ")[0] for h in headers}

        decompresed_reads = (sample.read_paths[0].with_suffix('.fq'),
                             sample.read_paths[1].with_suffix('.fq'))
        decompress_file(sample.read_paths[0], decompresed_reads[0])
        decompress_file(sample.read_paths[1], decompresed_reads[1])

        async with aiofiles.open(work_path / "unmapped_1.fq", "w") as f:
            async for header, seq, quality in read_fastq_from_path(decompresed_reads[0]):
                if header.split(" ")[0] in unmapped_roots:
                    await f.write("\n".join([header, seq, "+", quality]) + "\n")

        async with aiofiles.open(work_path / "unmapped_2.fq", "w") as f:
            async for header, seq, quality in read_fastq_from_path(decompresed_reads[1]):
                if header.split(" ")[0] in unmapped_roots:
                    await f.write("\n".join([header, seq, "+", quality]) + "\n")
...
@step
async def vfam(analysis: Analysis, hmms: HMMs, proc: int, results: dict, run_subprocess, work_path: Path, logger):
    """
    ...
    logger.info(list(hmms.path.glob('*')))
