...
@step
async def reunite_pairs(run_in_thread, sample: Sample, work_path: Path, logger):
    if sample.paired:
        ...
        for index in (0, 1):
            await run_in_thread(decompress_file, sample.read_paths[index], decompresed_reads[index])
